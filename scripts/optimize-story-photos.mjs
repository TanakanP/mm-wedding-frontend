...
const versions = Object.fromEntries(
  ids.map((id) => [id, Math.floor(statSync(join(outDir, `${id}.jpeg`)).mtimeMs)])
);

writeFileSync(
  join(root, "src/components/our-story/photo-versions.ts"),
  `// Generated by scripts/optimize-story-photos.mjs — do not edit\nexport const PHOTO_VERSIONS: Record<number, number> = ${JSON.stringify(versions, null, 2)};\n`
);
